...
void copy_hex(unsigned long num, char *buf, int *ptr) {
...
void copy_integer(int i,char *buf, int *ptr) {
	int d;
	int k = 0;
	char int_buffer[64];
	while(i>0) {
...
void kprintf_(const char *arg1,...) {
	...
	while(i<len) {
		...
		++i;
		switch(arg1[i]) {
			case 'c' : buffer[bufptr++] = (char)va_arg(ap,int);
						break;
			case 's' : st = (char *)va_arg(ap,char *);
						while(*st) {
							buffer[bufptr++] = *st++;
						}
						break;
			case 'd' : copy_integer(va_arg(ap,int),buffer,&bufptr);
						break;
			case 'x' :
