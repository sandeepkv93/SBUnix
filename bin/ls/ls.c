...
main(int argc, char* argv[])
{
    ...
    if (access(directory_name, F_OK) < 0) {
        ...
        while (1) {
            ...
            for (directory_position = 0; directory_position < nread;
                 directory_position += new_dirent_structure->d_reclen) {
                new_dirent_structure =
                  (struct new_dirent*)(dirent_buffer + directory_position);

                /* Get the file name from dirent structure */
                file_name = new_dirent_structure->d_name;

                if (strcmp(file_name, "..") && strcmp(file_name, ".")) {
                    d_type = *(dirent_buffer + directory_position +
                               new_dirent_structure->d_reclen - 1);
