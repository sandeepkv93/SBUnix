...
void
append(struct stringllnode** head_reference, char* new_data)
{
    struct stringllnode* new_node =
      (struct stringllnode*)malloc(sizeof(struct stringllnode));
...
void
print_list(struct stringllnode* node)
{
    while (node != NULL) {
        puts(node->data);
        node = node->next_node;
...
void
free_list(struct stringllnode* node)
{
