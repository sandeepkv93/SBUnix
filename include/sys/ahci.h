...
#define ATA_STATUS_ERR                                                         \
    0x01 // Indicates an error occurred. Send a new command to clear it (or nuke
         // it with a Software Reset).
#define ATA_STATUS_DRQ                                                         \
    0x08 // Set when the drive has PIO data to transfer, or is ready to accept
         // PIO data.
#define ATA_STATUS_SRV 0x10 // Overlapped Mode Service Request.
#define ATA_STATUS_DF 0x20  // Drive Fault Error (does not set ERR).
#define ATA_STATUS_RDY                                                         \
    0x40 // Bit is clear when drive is spun down, or after an error. Set
         // otherwise.
#define ATA_STATUS_BSY                                                         \
    0x80 // Indicates the drive is preparing to send/receive data (wait for it
         // to clear).
...
int read_ahci(hba_port_t *port, uint32_t startl, uint32_t starth, uint32_t count, uint16_t *buf);
int write_ahci(hba_port_t *port, uint32_t startl, uint32_t starth, uint32_t count, uint16_t *buf);
#endif
